...
# 读取数据文件
data = pd.read_csv('train_set.csv', dtype=float, on_bad_lines='warn', na_values=['#NAME?', 'NaN', ''])
...
# 准备预测数据
predict_data = pd.read_csv('pred_set.csv', dtype=float, on_bad_lines='warn', na_values=['#NAME?', 'NaN', ''])
