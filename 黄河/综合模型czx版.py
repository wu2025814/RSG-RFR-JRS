...
train_data = pd.read_csv("train_set3000.csv", low_memory=False)
test_data = pd.read_csv("pred_set1000.csv", low_memory=False)
...
plt.tight_layout()
plt.savefig("新模型1000.jpg", dpi=600)
plt.show()
...
result2_data = pd.read_csv('result1000.csv', low_memory=False)

# 确保 result2.csv 的行数与预测结果的数量相同
if len(result2_data) != len(y_pred_full):
    print(y_pred_full.size)
    raise ValueError("预测结果的数量与 result1000.csv 中的行数不匹配")

# 将预测结果添加为新列
result2_data['random_forest_depth'] = y_pred_full

# 保存更新后的 result1.csv 文件
result2_data.to_csv('result1000.csv', index=False)
print("Predictions added to result1.csv as 'random_forest_depth' column")
