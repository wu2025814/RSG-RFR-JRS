...
def get_regression_line(real, pred, data_range=(0, 12)):
    # 拟合（若换MK，自行操作）最小二乘
    def slope(xs, ys):
...
# 读取数据
data = pd.read_csv('result2000.csv')
...
# 设置统一的刻度和限制
xmin, xmax, ymin, ymax = 0, 12, 0, 12  # 修改为 [0, 12]
...
axs[0].plot([0, 12], [0, 12], 'k--', lw=1)  # 对角线范围调整为0-12
regression_line = get_regression_line(real1, pred1, data_range=(0, 12))  # 回归线范围调整为0-12
...
axs[0].text(1, 11, f'R$^2$={R_squared1:.2f}', ha='left', va='top', family='Arial', fontsize=12)
axs[0].text(1, 10, f'MAE={MAE1:.2f}m', ha='left', va='top', family='Arial', fontsize=12)
axs[0].text(1, 9, f'MRE={MRE1:.2f}', ha='left', va='top', family='Arial', fontsize=12)
axs[0].text(1, 8, f'RMSE={RMSE1:.2f}m', ha='left', va='top', family='Arial', fontsize=12)

# 绘制第二组图像
axs[1].scatter(real2, pred2, marker='o', c=z2*100, edgecolors=None, s=6.67,  # 数据点大小调整为原来的2/3
               label='LST', cmap='Spectral_r')
axs[1].plot([0, 12], [0, 12], 'k--', lw=1)  # 对角线范围调整为0-12
regression_line = get_regression_line(real2, pred2, data_range=(0, 12))  # 回归线范围调整为0-12
...
axs[1].text(1, 11, f'R$^2$={R_squared2:.2f}', ha='left', va='top', family='Arial', fontsize=12)
axs[1].text(1, 10, f'MAE={MAE2:.2f}m', ha='left', va='top', family='Arial', fontsize=12)
axs[1].text(1, 9, f'MRE={MRE2:.2f}', ha='left', va='top', family='Arial', fontsize=12)
axs[1].text(1, 8, f'RMSE={RMSE2:.2f}m', ha='left', va='top', family='Arial', fontsize=12)

# 绘制第三组图像
axs[2].scatter(real3, pred3, marker='o', c=z3*100, edgecolors=None, s=6.67,  # 数据点大小调整为原来的2/3
               label='LST', cmap='Spectral_r')
axs[2].plot([0, 12], [0, 12], 'k--', lw=1)  # 对角线范围调整为0-12
regression_line = get_regression_line(real3, pred3, data_range=(0, 12))  # 回归线范围调整为0-12
...
axs[2].text(1, 11, f'R$^2$={R_squared3:.2f}', ha='left', va='top', family='Arial', fontsize=12)
axs[2].text(1, 10, f'MAE={MAE3:.2f}m', ha='left', va='top', family='Arial', fontsize=12)
axs[2].text(1, 9, f'MRE={MRE3:.2f}', ha='left', va='top', family='Arial', fontsize=12)
axs[2].text(1, 8, f'RMSE={RMSE3:.2f}m', ha='left', va='top', family='Arial', fontsize=12)

# 绘制第四组图像
axs[3].scatter(real4, pred4, marker='o', c=z4*100, edgecolors=None, s=6.67,  # 数据点大小调整为原来的2/3
               label='LST', cmap='Spectral_r')
axs[3].plot([0, 12], [0, 12], 'k--', lw=1)  # 对角线范围调整为0-12
regression_line = get_regression_line(real4, pred4, data_range=(0, 12))  # 回归线范围调整为0-12
...
axs[3].text(1, 11, f'R$^2$={R_squared4:.2f}', ha='left', va='top', family='Arial', fontsize=12)
axs[3].text(1, 10, f'MAE={MAE4:.2f}m', ha='left', va='top', family='Arial', fontsize=12)
axs[3].text(1, 9, f'MRE={MRE4:.2f}', ha='left', va='top', family='Arial', fontsize=12)
axs[3].text(1, 8, f'RMSE={RMSE4:.2f}m', ha='left', va='top', family='Arial', fontsize=12)

# 统一设置XY轴刻度和限制
for ax in axs:
    ax.set_xlim(xmin, xmax)
    ax.set_ylim(ymin, ymax)
    ax.set_xticks(np.arange(xmin, xmax+1, 2))  # 例如，每2单位一个刻度
...
plt.savefig("comparison_models2000.jpg", dpi=600)