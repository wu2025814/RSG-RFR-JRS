...
# 读取数据文件
data = pd.read_csv('train_set3000.csv', dtype=float, on_bad_lines='warn', na_values=['#NAME?', 'NaN', ''])
...
# 准备预测数据
predict_data = pd.read_csv('pred_set1000.csv', dtype=float, on_bad_lines='warn', na_values=['#NAME?', 'NaN', ''])
...
# 加载现有的CSV文件并确保长度匹配
existing_data = pd.read_csv('result1000.csv', dtype=float, on_bad_lines='warn', na_values=['#NAME?', 'NaN', ''])
...
existing_data.to_csv('result1000.csv', index=False)
