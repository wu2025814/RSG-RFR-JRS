...
# 读取数据文件，并将无效值设置为 NaN
data = pd.read_csv('train_set2000.csv', na_values=['#NAME?', ''])
...
# 准备预测数据
predict_data = pd.read_csv('pred_set2000.csv', na_values=['#NAME?', ''])
...
b_columns = [f'Band_{i}' for i in range(1, 5)]
...
# 加载现有的 CSV 文件
existing_data = pd.read_csv('result2001.csv')
...
existing_data.to_csv('result2001.csv', index=False)
