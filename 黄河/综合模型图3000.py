...
plt.tight_layout()
plt.savefig("新模型3001.jpg", dpi=600)
plt.show()
...
result2_data = pd.read_csv('result3001.csv')

# 确保 result2.csv 的行数与预测结果的数量相同
if len(result2_data) != len(y_pred_full):
    print(y_pred_full.size)
    print(result2_data.size)
    raise ValueError("预测结果的数量与 result3001.csv 中的行数不匹配")

# 将预测结果添加为新列
result2_data['random_forest_depth'] = y_pred_full

# 保存更新后的 result2.csv 文件
result2_data.to_csv('result3001.csv', index=False)
print("Predictions added to result3001.csv as 'random_forest_depth' column")
